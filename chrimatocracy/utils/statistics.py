...
def donations_received_stats(df: pd.DataFrame, year: int, role:str, table_path: Path):
    ...
    stats_donations = pd.DataFrame(df["num_donation_ammount"].describe()).T.append(
        pd.DataFrame(
            df[
                df["id_donator_effective_cpf_cnpj"].astype(str).apply(lambda x: len(x)) == 14
            ]["num_donation_ammount"].describe()
        ).T.append(
            pd.DataFrame(
                df[
                    df["id_donator_effective_cpf_cnpj"].astype(str).apply(lambda x: len(x)) == 11
                ]["num_donation_ammount"].describe()
            ).T
        )
    )
...
def donations_made_stats(df: pd.DataFrame, by: str, year: int, role:str, table_path:Path):
    col_map = dict(companies = "cat_original_donator_name2",
    ...
    petry_name_map = dict(companies = "Company Name",
