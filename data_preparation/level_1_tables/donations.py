...
receitas_comites_brasil = pd.read_csv(raw_data_path / "receitas_comites_2014_brasil.txt",
                             encoding="ISO-8859-1", sep=';', na_values='#NULO', dtype=str)
receitas_partidos_brasil = pd.read_csv(raw_data_path / "receitas_partidos_2014_brasil.txt",
                              encoding="ISO-8859-1", sep=';', na_values='#NULO', dtype=str)
receitas_candidatos_brasil = pd.read_csv(raw_data_path / "receitas_candidatos_2014_brasil.txt",
                               encoding="ISO-8859-1", sep=';', na_values='#NULO', dtype=str)
print('Done.')
...
receitas_comites_brasil.loc[:, 'num_donation_ammount'] = receitas_comites_brasil['num_donation_ammount'].str.replace(
    ',', '.').apply(float)
receitas_partidos_brasil.loc[:, 'num_donation_ammount'] = receitas_partidos_brasil['num_donation_ammount'].str.replace(
    ',', '.').apply(float)
receitas_candidatos_brasil.loc[:, 'num_donation_ammount'] = receitas_candidatos_brasil['num_donation_ammount'].str.replace(
    ',', '.').apply(float)
print('Done.')

print('Formating role names...')
receitas_candidatos_brasil.loc[:, 'cat_role_description'] = receitas_candidatos_brasil['cat_role_description'].str.upper()
print('Done.')
...
receitas_candidatos_brasil.loc[:, "cat_original_donator_name2"] = receitas_candidatos_brasil.cat_original_donator_name2.apply(
    lambda x: unidecode.unidecode(x).replace('/', '').replace('.', '').strip() if type(x) == str else x)
receitas_candidatos_brasil.loc[:, "cat_donator_economic_sector"] = receitas_candidatos_brasil.cat_donator_economic_sector.apply(
    lambda x: unidecode.unidecode(x).replace('/', '').replace('.', '').strip() if type(x) == str else x)
...
receitas_comites_brasil.to_csv(
    data_path / "brazil_2014_donations_committees.csv", index=False)
receitas_partidos_brasil.to_csv(
    data_path / "brazil_2014_donations_parties.csv", index=False)
receitas_candidatos_brasil.to_csv(
    data_path / "brazil_2014_donations_candidates.csv", index=False)
print('Done.')
