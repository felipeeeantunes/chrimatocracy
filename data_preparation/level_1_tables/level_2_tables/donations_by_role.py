...
receitas_candidatos_brasil = pd.read_csv(data_path / "brazil_2014_donations_candidates.csv", low_memory=False, dtype=data_types)


roles = [x.upper() for x in receitas_candidatos_brasil["cat_role_description"].unique()]
translator = {"GOVERNADOR": "governor",
              "SENADOR": "senator",
...
for role in roles:
    ...
    receitas.to_csv(data_path / f'brazil_2014_{translated_role}_candidates_donations.csv', index=False)
    print('Done.')

    print(f"Aggregating donations table for {role}.\n")
    g_candidates = receitas.groupby([u'id_candidate_cpf', 'id_donator_effective_cpf_cnpj','cat_party', 'cat_role_description', 'cat_federative_unity']).agg({'num_donation_ammount': 'sum'})
    g_candidates = pd.DataFrame(g_candidates)
    g_candidates = g_candidates.reset_index()
    g_candidates.to_csv(data_path / f'brazil_2014_{translated_role}_candidates_donations_aggregated.csv', index=False)
