...
for role in roles:
    for situation in situations:
        ...
        candidatos = pd.read_csv(data_path / f'brazil_2014_{role}_{situation}_candidates.csv', dtype=data_types, low_memory=False)
        votos      = pd.read_csv(data_path / f'brazil_2014_{role}_{situation}_candidates_votes.csv', dtype=data_types, low_memory=False)
        ...
        # %%
        merge_index = ['id_candidate_sequential', 'id_candidate_number','int_election_turn', 'cat_federative_unity', 'cat_state_unity', 'id_role_code']
        grouped_votos_candidates_municipios = votos.groupby(merge_index).agg({'int_number_of_votes': sum})
        try:
            merged_votos_candidatos = pd.merge( candidatos.set_index(merge_index),
