...
for role in roles:
    ...
    receitas_candidatos = pd.read_csv(
        data_path / f"brazil_2014_{role}_candidates_donations_aggregated.csv", dtype=data_types, low_memory=False
    )
    votos_candidatos = pd.read_csv(
        data_path / f"brazil_2014_{role}_accepted_candidates_votes_with_candidates_info.csv",
        dtype=data_types,
        low_memory=False,
    )

    list_doador = receitas_candidatos["id_candidate_cpf"].unique()
    ...
    try:
        merged_receitas = pd.merge(
            receitas_candidatos, votos_candidatos, left_on=merge_index, right_on=merge_index, how="outer", sort=False
        )
        merged_receitas["num_donation_ammount"].fillna(0, inplace=True)
        merged_receitas["int_number_of_votes"].fillna(0, inplace=True)
        merged_receitas.to_csv(
            data_path / f"brazil_2014_{role}_accepted_candidates_votes_with_candidate_info_and_donations.csv",
            index=False,
        )
    except:
